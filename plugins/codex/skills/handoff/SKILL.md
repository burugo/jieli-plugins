---
name: handoff
description: "Compress the current Codex session into a handoff prompt for a fresh agent, embedding the current Jieli thread id so the next agent can read the thread for full context. Use for context handoff, continuing in a new session, or when the context window is filling up."
---
...
- Base URL from `JIELI_BASE_URL` or `~/.config/jieli/settings.json`. If omitted, use `https://jieli.app`. No API key is needed; the handoff only builds a thread URL and never calls the Jieli API.

## Procedure

### 1. Detect the current thread id

The session is auto-synced to Jieli by the sync hooks, and its thread id is `T-<session_id>`. Resolve the base URL and the thread id from the newest Codex rollout whose `session_meta.cwd` matches the current working directory:
...
THREAD_ID="$(python3 - "$PWD" <<'PY'
import sys, os, json, pathlib
cwd = sys.argv[1]
roots = []
if os.environ.get("CODEX_HOME"):
    roots.append(pathlib.Path(os.environ["CODEX_HOME"]).expanduser() / "sessions")
home = pathlib.Path.home()
roots += [home/"Library"/"Application Support"/"Codex"/"sessions", home/".codex"/"sessions"]
cands = []
for r in roots:
    if r.exists():
        cands += list(r.rglob("rollout-*.jsonl"))
...
- If `THREAD_ID` is empty, sync may not be configured. Still produce the handoff prompt, but tell the user the thread id could not be detected; ask them to configure the plugin (so the next agent can read the thread) or to paste the thread id / Jieli URL manually. Use `OUT="${TMPDIR:-/tmp}/handoff-unknown.md"` and drop the "read thread" line.
...
- The current turn is uploaded to Jieli by the sync hooks after this turn ends, so reading the thread covers history up to the previous sync. The handoff summary itself already carries the key context, so this gap is fine.
