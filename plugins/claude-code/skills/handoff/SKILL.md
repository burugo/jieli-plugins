...
If that command is not found or does not print valid JSON, locate and run the installed helper from the Claude Code plugin cache instead of guessing a thread id:
...
const roots = [
  path.join(os.homedir(), ".claude", "plugins", "cache", "jieliapp"),
  path.join(os.homedir(), ".claude", "plugins", "cache", "jieli"),
];
const helpers = [];
function walk(dir) {
  let entries = [];
